...
///
/// Reference: https://developer.android.com/reference/android/bluetooth/le/BluetoothLeAdvertiser
@JsonSerializable()
class AndroidAdvertiseSettings {
  ...
  /// Used when NOT using extended advertising (Android < 8 or when extended is not needed).
  ///
  /// If both [advertiseSettings] and [advertiseSetParameters] are null,
  /// defaults to legacy advertising with default settings.
  ///
  /// Android API: `startAdvertising(AdvertiseSettings, AdvertiseData, AdvertiseCallback)`
  ...
  /// When provided, uses the extended advertising API instead of legacy.
  ///
  ...
  ///
  /// Additional advertising data sent in response to scan requests.
  /// Useful when the main advertising packet is full.
  ///
  /// Maps to the `scanResponse` parameter in:
  /// - Legacy: `startAdvertising(..., scanResponse, ...)`
  /// - Extended: `startAdvertisingSet(..., scanResponse, ...)`
  ///
  /// **Important**: Uses [AndroidAdvertiseData] to support all Android AdvertiseData fields
  /// including manufacturer data, service data, service solicitation UUIDs, etc.
  ///
  ...
  ///
  /// Data to broadcast periodically after initial advertising.
  /// Only used with extended advertising ([advertiseSetParameters] must be set).
  ///
  ...
  ///
  /// Controls the interval and TX power for periodic advertising.
  /// Only used when [periodicAdvertiseData] is provided.
  ///
  ...
  ///
  /// Returns true if [advertiseSetParameters] is set, indicating
  /// the extended advertising API (Android 8+) should be used.
